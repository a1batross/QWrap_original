...
{
	return mx::regCreateKey( HKEY_CURRENT_USER, "Software\\XashXT Group\\Quake 1 ModelViewer" );
}

bool SaveString( const char *pKey, char *pValue )
{
	return mx::regSetValue( HKEY_CURRENT_USER, "Software\\XashXT Group\\Quake 1 ModelViewer", pKey, pValue );
}

bool LoadString( const char *pKey, char *pValue )
{
	return mx::regGetValue( HKEY_CURRENT_USER, "Software\\XashXT Group\\Quake 1 ModelViewer", pKey, pValue );
}
...
bool IsStudioModel( const char *path )
{
	byte		buffer[256];
	FILE		*fp;

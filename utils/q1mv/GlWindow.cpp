...
#endif
		g_aliasModel.updateTimings( curr, dt );
		...
		if( !g_bStopPlaying && !g_viewerSettings.pause && prev != 0.0 )
		{
			...
			if( d_pol >= 1.0f )
			{
				if( d_startFrame == d_endFrame )
					g_aliasModel.setFrame( d_startFrame );
				else g_aliasModel.setFrame( d_currFrame++ );
				d_pol = 0.0f;
...
void GlWindow :: drawFloor( int texture )
{
	...
	if( texture ) 
	{
		...
		g_aliasModel.GetMovement( deltaPos );
...
void GlWindow::draw ()
{
	...
	if( g_viewerSettings.showTexture )
	{
		...
		glLoadIdentity();
		glOrtho( 0.0f, w2(), h2(), 0.0f, 1.0f, -1.0f );
		glDisable( GL_MULTISAMPLE );
		...
		if( hdr )
		{
			float w = (float) hdr->skinwidth * g_viewerSettings.textureScale;
			float h = (float) hdr->skinheight * g_viewerSettings.textureScale;
			...
			else
			{
				...
				glBindTexture (GL_TEXTURE_2D, g_aliasModel.GetTextureSkin( g_viewerSettings.texture, g_viewerSettings.skinAnim ));
			}
			...
			if( g_viewerSettings.show_uv_map || g_viewerSettings.pending_export_uvmap || g_viewerSettings.overlay_uv_map )
			{
				...
				g_aliasModel.SetOffset2D( x, y );
				g_aliasModel.DrawAliasUVMap();
...
int GlWindow :: loadModel( const char *filename, bool centering )
{
	g_aliasModel.FreeModel ();
	if( g_aliasModel.LoadModel( filename ))
	{
...
int GlWindow :: loadTextureImage( mxImage *image, int name )
{
	if( image )
	{
		...
		if( image->bpp == 8 )
		{
			g_aliasModel.UploadTexture( (byte *)image->data, image->width, image->height, (byte *)image->palette, name );
		}
